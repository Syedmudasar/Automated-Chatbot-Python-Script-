...
class Test84(unittest.TestCase):
    ...
    def test84_verify_select_date_from_drop_down(self):
        ...
        try:
            ...
            time.sleep(3)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('change_date')).click()
            logger.info('User has changed the date')

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('select_date_drop_box')).click()
            logger.info('User has clicked on select date box')

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('start_last_week_date')).click()
            logger.info('User has clicked on start last week date')

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('next_day_date')).click()
            logger.info('User has also clicked on next day\'s date')
