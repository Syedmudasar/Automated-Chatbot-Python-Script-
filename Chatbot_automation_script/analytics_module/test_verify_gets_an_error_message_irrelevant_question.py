...
class Test91(unittest.TestCase):
    ...
    def test91_verify_gets_a_message_entered_irrelevant_data_in_search_field(self):
        ...
        try:
            ...
            search_input.send_keys(ExcelReader.read_excel2().get('irrelevant_question'))
            ...
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('error_message1')).is_displayed()

            message = driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('error_message1')).text
            logger.info('User has got a message ::: {}'.format(message))

        except Exception as e:
            allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
            logger.error('An Exception occurred in test case verify gets an error message 91')
            print(e)
            pytest.fail()