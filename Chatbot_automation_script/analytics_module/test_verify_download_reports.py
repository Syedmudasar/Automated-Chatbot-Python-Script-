...
class Test87(unittest.TestCase):
    ...
    def test87_verify_download_reports_previously_weekly_monthly(self):
        ...
        try:
            ...
            time.sleep(2)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('conversation_counter1')).click()
            logger.info('User has opened all conversation on current date 2')

            time.sleep(3)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('download_icon')).click()
            logger.info('User has clicked on download button to download current report 2')

            time.sleep(2)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('popup_close')).click()
            logger.info('User has closed the conversation reports popup 2')

            # validate that user is able to see download reports monthly
            ...
            time.sleep(3)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('download_icon')).click()
            logger.info('User has clicked on download button to download current report 3')
            ...
            logger.error('An Exception occurred in test case verify download reports 87')
            print(e)
            pytest.fail()