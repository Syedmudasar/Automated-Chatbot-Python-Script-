...
class Test19:
    ...
    def test19_Verify_an_error_message_email_address_blank(self):
        try:
            ...
            time.sleep(2)
            flag = self.driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('send_invitation')).is_enabled()

            if flag:
                logger.info('as expected invitation button is disabled')
            else:
                logger.info('invitation button is enabled as not expected')


        except Exception as e:
            allure.attach(self.driver.get_screenshot_as_png(), 'screenshots',  AttachmentType.PNG)
            pytest.fail(e, pytrace=True)
            logger.error('An Error occurred in Verify_an_error_message_email_address_blank 19')
            print(e)
