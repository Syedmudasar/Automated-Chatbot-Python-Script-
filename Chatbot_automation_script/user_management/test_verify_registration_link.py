...
class Test25:
    ...
    def test25_verify_registration_link(self):
        try:
            ...
            element = self.driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('create_account_link'))
            href_data = element.get_attribute('href')
            if href_data is None:
                logger.info('Link is not click able')
            else:
                logger.info('Link is click able')
            ...
            logger.error('An Error occurred in Verify quick action link 25')
            print(e)
