...
class Test15:
    ...
    def test15_Validate_Invalidate_Token_functionality_under_Invited_tab(self):
        try:
            ...
            logger.info('User has successfully clicked on invite tab from tab list')
            ...
            logger.info('Invalidate Pop up message is visible there')

        except Exception as e:
            allure.attach(self.driver.get_screenshot_as_png(), 'screenshots',  AttachmentType.PNG)
            logger.error('An Error occurred in verify Invalidate functionality')
            print(e)
