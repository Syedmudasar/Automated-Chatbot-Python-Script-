...
class Test131(unittest.TestCase):
    ...
    def test131_verify_train_bots_functionality_under_setting_module(self):
        ...
        try:
            ...
            flag = driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('train_bots')).is_enabled()
            if flag:
                logger.info('Train bots is clickable')


            time.sleep(3)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('train_bots')).click()
            logger.info('User has clicked on train bots functionality 1')

            time.sleep(3)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('train_bots1')).click()
            logger.info('User has clicked on train bot functionality 2')

            time.sleep(2)

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('training_joined')).is_displayed()
            logger.info('Training has been joining...........')

            time.sleep(3)

        except Exception as e:
            allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
            logger.error('An Exception occurred in test case verify train bots functionality under setting module 131')
            print(e)
            pytest.fail()