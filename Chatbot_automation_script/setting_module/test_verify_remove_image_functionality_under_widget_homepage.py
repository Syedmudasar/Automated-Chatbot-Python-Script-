...
class Test129(unittest.TestCase):
    ...
    def test129_verify_remove_image_functionality_under_widget_home_page(self):
        ...
        try:
            ...
            if flag:
                logger.info('Confirmation Pop is visible after click on remove button')

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('confirmation_button')).click()
            logger.info('User has clicked on Yes button to remove image')

            time.sleep(3)

        except Exception as e:
            allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
            logger.error('An Exception occurred in test case verify remove image functionality under widget homepage 129')
            print(e)
            pytest.fail()