...
class Test119(unittest.TestCase):
    ...
    def test119_verify_data_after_clicking_view_release_note_under_setting_module(self):
        ...
        try:
            ...
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('view_release_note')).click()
            logger.info('User has clicked on view release under setting module')


            url = ExcelReader.read_excel().get('hrbot_release_notes_url')
            print(url)
            ...
            print(windows)
            Utility.switch_windows(windows, driver)
            ...
            logger.info('User has been navigated to new windows and release note page')

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('release_note')).is_displayed()
            logger.info('Release Note Heading is visible there')

            release_version = driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('new_version')).text
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('new_version')).is_displayed()
            logger.info('New version is visible there as ::: {}'.format(release_version))

            old_version = driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('old_version')).text
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('old_version')).is_displayed()
            logger.info('Old version also is visible there as ::: {}'.format(old_version))

            time.sleep(4)
        except Exception as e:
            allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
            logger.error('An Exception occurred in test case verify data after clicking on view release note under setting module 119')
            print(e)
            pytest.fail()