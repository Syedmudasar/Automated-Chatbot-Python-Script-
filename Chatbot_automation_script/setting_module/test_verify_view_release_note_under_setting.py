...
class Test118(unittest.TestCase):
    ...
    def test118_verify_view_release_note_under_setting_module(self):
        ...
        try:
            ...
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('view_release_note')).is_displayed()
            logger.info('View release note is displayed under setting module')

        except Exception as e:
            allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
            logger.error('An Exception occurred in test case verify view release note under setting module 118')
            print(e)
            pytest.fail()