...
class Test109(unittest.TestCase):
    ...
    for i in range(3):
        def test109_verify_cancel_button_instead_delete_any_interaction(self):
            ...
            try:
                ...
                time.sleep(3)
                flag = driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('delete_icon')).is_displayed()
                if flag:
                    ...
                    logger.info('User has clicked on cancel button')
                else:
                    logger.info('Delete icon is not visible it means there is not template')

            except Exception as e:
                allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
                logger.error('An Exception occurred in test case verify if press cancel button while deleting any interaction 109')
                print(e)
                pytest.fail()