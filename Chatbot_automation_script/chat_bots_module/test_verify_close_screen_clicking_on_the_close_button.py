...
class Test114(unittest.TestCase):
    ...
    def test114_verify_close_the_screen_by_clicking_on_close_button(self):
        ...
        try:
            ...
            Utility.explicit_Wait1(HrBot_Config_mapper.hrbot_locators.get('import_template_popup').format(region_name),
                                   'visibility', driver)
            ...
            logger.info('User has clicked into search input box')

            search_element = driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('search_database_input'))
            search_element.send_keys(ExcelReader.read_excel2().get('search_template'))
            logger.info('User has enter a database name into search input box')

            time.sleep(2)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('searched_database')).click()
            logger.info('User has selected a database to import file into that')

            time.sleep(4)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('close_import_screen')).click()
            logger.info('User has clicked on close button')

            time.sleep(4)

        except Exception as e:
            allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
            logger.error('An Exception occurred in test case verify import file by search in database  114')
            print(e)
            pytest.fail()