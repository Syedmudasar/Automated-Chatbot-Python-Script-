...
class Test111(unittest.TestCase):
    ...
    for i in range(3):
        def test110_verify_update_to_bots_icon(self):
            ...
            try:
                ...
                if region_name in ['Small Talk', 'TICA-El Salvador', 'TIC-Canada', 'TIE-Ireland', 'TIE-Romania',
                                   'TI-Philippines', 'TIUS-USA']:
                    ...
                    region_type = Utility.choose_random_region2(region_number, xpath, driver)
                else:
                ...
                for i in range(5):
                    if region_type in ['Small Talk', 'TICA-El Salvador', 'TIC-Canada', 'TIE-Ireland', 'TIE-Romania',
                        ...
                        Utility.explicit_Wait1(HrBot_Config_mapper.hrbot_locators.get('relationship_red_icon'),
                                               'visibility', driver)

                        flag = driver.find_element_by_xpath(
                            HrBot_Config_mapper.hrbot_locators.get('relationship_red_icon')).is_displayed()
                        if flag:
                            driver.find_element_by_xpath(
                                HrBot_Config_mapper.hrbot_locators.get('update_to_bots')).click()
                            logger.info('User has clicked on update to bots icon')

                            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('bot_updated_message')).is_displayed()
                            logger.info('Template is updated successfully')
                            break
                    else:
                        time.sleep(4)
                        driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('open_drop_down1')).click()
                        ...
                        if flag:
                            driver.find_element_by_xpath(
                                HrBot_Config_mapper.hrbot_locators.get('update_to_bots')).click()
                            logger.info('Template is updated to bots')
                            time.sleep(3)

                            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('bot_updated_message')).is_displayed()
                            logger.info('Template is updated to bots and confirmation pop up is popped up')
                            break

            except Exception as e:
                allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
                logger.error('An Exception occurred in test case verify update to bots icon 111')
                print(e)
                pytest.fail()