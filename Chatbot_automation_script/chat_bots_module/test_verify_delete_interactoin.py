...
class Test108(unittest.TestCase):
    ...
    def test108_verify_delete_any_interaction(self):
        ...
        try:
            ...
            Utility.choose_random_region(region_number, xpath, driver)

            time.sleep(3)

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('delete_icon')).click()
            logger.info('User has clicked on delete icon')

            Utility.explicit_Wait1(HrBot_Config_mapper.hrbot_locators.get('confirmation_windows'), 'visibility', driver)

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('confirmation_windows')).is_displayed()
            logger.info('Confirmation pop up is visible before delete interaction')

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('delete')).click()
            logger.info('User has deleted an interaction')

            Utility.explicit_Wait1(HrBot_Config_mapper.hrbot_locators.get('template_deleted'), 'visibility', driver)
            
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('template_deleted')).is_displayed()
            logger.info('User has got confirmation message template deleted successfully')

        except Exception as e:
            allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
            logger.error('An Exception occurred in test case verify delete any interaction 108')
            print(e)
            pytest.fail()