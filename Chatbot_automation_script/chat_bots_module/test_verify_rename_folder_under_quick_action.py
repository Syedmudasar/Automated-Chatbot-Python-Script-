...
class Test104(unittest.TestCase):
    ...
    def test104_verify_rename_folder_under_quick_action_on_chat_bots_page(self):
        ...
        try:
            ...
            for names in list_folder_names1:
                ...
                if folder == names:
                    ...
                    try:
                        ...
                        time.sleep(4)
                        path2 = HrBot_Config_mapper.hrbot_locators.get('rename_folder')
            ...
            logger.info('User has entered sub folder name as ::: '.format(ExcelReader.read_excel2().get('rename_sub_folder')))

            time.sleep(3)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('update_folder_name')).click()
            logger.info('User has deleted the folder successfully')

            time.sleep(3)

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('renamed_folder_pup')).is_displayed()
            logger.info('Confirmation pop up is displayed on the right corner of application')
            # time.sleep(3)
            ...
            logger.error('An Exception occurred in test case verify rename folder under quick action 104')
            print(e)
            pytest.fail()