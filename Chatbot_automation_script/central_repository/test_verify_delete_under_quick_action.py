...
class Test64:
    ...
    def test64_verify_delete_option_under_quick_action_on_central_repository(self):
        try:
            driver = self.driver
            ...
            flag = driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('delete_under_quick')).is_displayed()
            assert flag == True
            logger.info('Delete option is visible under quick action')

        except Exception as e:
            allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
            logger.error('An Exception occurred in test case verify delete option under quick action on central repository 64')
            print(e)
