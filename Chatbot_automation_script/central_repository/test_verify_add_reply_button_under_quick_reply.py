...
class Test148(unittest.TestCase):

    def test148_Verify_add_reply_under_quick_reply_on_interaction_page(self):
        ...
        try:
            ...
            time.sleep(2)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('quick_reply_input_box')).click()
            logger.info('User has clicked in quick reply input box')

            time.sleep(1)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('quick_reply_input_box')).send_keys(ExcelReader.read_excel1().get('answer_by_quick_reply'))
            logger.info('User has insert quick reply answer into quick reply')

            time.sleep(2)

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('add_title_name_button')).click()
            logger.info('User has clicked on add title hare button to add title in quick reply')

            time.sleep(2)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('enter_title_name')).click()
            logger.info('User has clicked in title input box')
            ...
            logger.info('User has insert title name into quick reply')

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('enter_query')).click()
            logger.info('User has clicked in enter query input box')
            ...
            logger.info('User has insert a query into quick reply')

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('add_button1'))
            logger.info('User has clicked on add button')

            time.sleep(2)

            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('add_reply')).click()
            logger.info('User has clicked on add reply button')
            ...
            logger.info('An exception occurred in verify add reply under quick reply on interaction page flow repository 148')
