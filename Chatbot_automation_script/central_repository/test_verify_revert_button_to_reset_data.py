...
class Test56:
    ...
    def test56_verify_user_allowed_reset_template_data_using_revert_functionality(self):
        try:
            driver = self.driver
            ...
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('revert_button')).click()
            logger.info('User has clicked on revert button')

            time.sleep(3)
            flag = driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('answers_box')).is_displayed()
            assert flag == False
            logger.info('Text Answer box is not visible means revert button is working fine ')

        except Exception as e:
            allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
            logger.error('An Exception occurred in test case verify user allowed to reset data using revert functionality 56')
            print(e)
