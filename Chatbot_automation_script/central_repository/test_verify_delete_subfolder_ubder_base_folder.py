...
class Test71:
    ...
    def test71_verify_delete_sub_folder_under_base_folder_on_central_repository(self):
        try:
            driver = self.driver
            ...
            for names in list_folder_names2:
                ...
                if folder == names:
                    ...
                    try:
                        ...
                        time.sleep(4)
                        x = "((//*[@class='rst__rowToolbar'])[{}]//div[@class='rst__rowNodeDelete'])/span[@class='text-btnTitle']".format(counter)
                        driver.find_element_by_xpath(x).click()
                        # action.move_to_element(element2).click(element2)
                        time.sleep(5)
                        break
            ...
            time.sleep(3)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('delete_folder')).click()
            logger.info('User has deleted sub folder under base folder')

            time.sleep(1)
            driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('deleted_folder')).is_displayed()
            logger.info('Sub folder is deleted successfully')


        except Exception as e:
            allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
            logger.error('An Exception occurred in test case verify delete sub folder under base folder on central repository 71')
            print(e)
