...
class Test57:
    ...
    def test57_verify_paste_card_button_visible(self):
        try:
            driver = self.driver
            ...
            time.sleep(3)
            flag = driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('paste_card')).is_displayed()
            assert flag == True
            logger.info('Paste card is visible means user is able to see paste card')

        except Exception as e:
            allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
            logger.error('An Exception occurred in test case verify user is bale to see faste card 57')
            print(e)
