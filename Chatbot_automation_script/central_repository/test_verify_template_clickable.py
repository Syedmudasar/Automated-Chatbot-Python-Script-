...
class Test42:
    ...
    def test42_verify_anyone_template_clickable(self):
        ...
        try:
            ...
            flag = driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('template_name')).is_enabled()
            assert flag == True
            logger.info('The template is clickable')

        except Exception as e:
            allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
            logger.error('An Exception occurred in test case verify template clickable or not 42')
            print(e)
