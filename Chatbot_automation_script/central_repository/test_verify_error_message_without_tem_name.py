...
class Test44:
    ...
    def test44_Verify_error_message_without_template_name(self):
        try:
            driver = self.driver
            ...
            time.sleep(2)
            message = driver.find_element_by_xpath(HrBot_Config_mapper.hrbot_locators.get('empty_temp_name_error_message')).text
            assert message == "Title for the template can't be empty."
            logger.info('User gets an error message if temp name is empty')


        except Exception as e:
            allure.attach(driver.get_screenshot_as_png(), 'screenshots', AttachmentType.PNG)
            logger.error('An Exception occured in test case verify error message without putting template name 44')
            print(e)
